//! Thin PyO3 binding for the `image::psf` orchestrators and leaves.
//!
...
//! through unchanged.
//!
//! Surface: the nested `noobase._core.image.psf` submodule with
//! `robust_combine`, `solve_flux_background`, `build_epsf`,
//! `stitch_psf`, `build_extended_psf`, and their getter pyclasses.
//!
//! `build_stamp` / `StampResult` live one level up in `noobase._core.image`
//! (mirroring the Rust core `image::stamp` leaf) -- see the sibling
//! `stamp` module.
//!
...
use crate::convert::{
    Scalar, dispatch_array, optional_companion_array3, optional_f64_array2, parse_combine_method,
    parse_residual_reweight, required_f64_array2, to_value_error,
};
...
pub(crate) fn build_submodule<'py>(py: Python<'py>, image: &Bound<'py, PyModule>) -> PyResult<()> {
